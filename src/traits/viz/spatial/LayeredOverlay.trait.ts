 ...
 */
export const BASEMAP_TYPES = ['none', 'tile', 'boundaries'] as const;

export type BasemapType = (typeof BASEMAP_TYPES)[number];

/**
 * Layer type for overlay layers
 */
...
const LayeredOverlayTrait = {
  trait: {
    name: 'LayeredOverlay',
    version: '1.0.0',
    description:
      'Supports stacking multiple visualization layers on a single map base. Enables composition of choropleth, symbol, route, and other layer types with deterministic z-ordering.',
    category: 'viz.spatial',
    tags: ['viz', 'spatial', 'geo', 'layers', 'map', 'composition'],
  },
  ...
  parameters: [
    {
      ...
      default: 'boundaries',
      enumValues: [...BASEMAP_TYPES],
    },
    {
      name: 'layers',
      type: 'string', // Stored as JSON array string
      required: false,
  ...
  metadata: {
    ...
    examples: ['MultiLayerMap', 'ChoroplethWithSymbols', 'RouteMap'],
    references: [
